export default {
  ...
  // Jarvis-Judice-Ninke error diffusion — 12 neighbors, very smooth
  // Weights: 7/48, 5/48, 3/48, 1/48
  ...
  processFn: (imageData, params) => {
    ...
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        ...
        // JJN 12-neighbor kernel
        if (x + 1 < w) lums[(x + 1) + y * w] += err * (7.0 / 48.0);
        if (x + 2 < w) lums[(x + 2) + y * w] += err * (5.0 / 48.0);
        if (x - 2 >= 0 && y + 1 < h) lums[(x - 2) + (y + 1) * w] += err * (3.0 / 48.0);
        if (x - 1 >= 0 && y + 1 < h) lums[(x - 1) + (y + 1) * w] += err * (5.0 / 48.0);
        if (y + 1 < h) lums[x + (y + 1) * w] += err * (7.0 / 48.0);
        if (x + 1 < w && y + 1 < h) lums[(x + 1) + (y + 1) * w] += err * (5.0 / 48.0);
        if (x + 2 < w && y + 1 < h) lums[(x + 2) + (y + 1) * w] += err * (3.0 / 48.0);
        if (x - 2 >= 0 && y + 2 < h) lums[(x - 2) + (y + 2) * w] += err * (1.0 / 48.0);
        if (x - 1 >= 0 && y + 2 < h) lums[(x - 1) + (y + 2) * w] += err * (3.0 / 48.0);
        if (y + 2 < h) lums[x + (y + 2) * w] += err * (5.0 / 48.0);
        if (x + 1 < w && y + 2 < h) lums[(x + 1) + (y + 2) * w] += err * (3.0 / 48.0);
        if (x + 2 < w && y + 2 < h) lums[(x + 2) + (y + 2) * w] += err * (1.0 / 48.0);
